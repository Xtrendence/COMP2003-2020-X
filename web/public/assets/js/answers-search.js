	...
	verifySession(sessionToken).then(result => {
        ...
        try {
            ...
            function displayInfo() {
                ...
                } else if (customTab.classList.contains("active")) {
            ...
            function allQuestionInfo () {
                ...
                xhr.open("GET", "./api/questions/read-all.php?key=" + sessionToken + "", true);
                xhr.send();
                findQuestionInfo(allQuestions)
            }

            function findQuestionInfo (allQuestions) {
                let xhr = new XMLHttpRequest();
                xhr.addEventListener("readystatechange", function() {
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        try {
                            ...
                            for (let i = 0; i < answeredQuestions.length; i++) {
                                for (let j = 0; j < allQuestions.length; j++) {
                                    if (answeredQuestions[i] == allQuestions[j]) {
                                        allQuestions.splice(j);
                                    }
                                }
                            }
                            console.log(allQuestions);
